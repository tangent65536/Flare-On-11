...
class Parser:
  ...
  def __init__(self, sc, base_addr):
    ...
    self.recover_trimmed_lea_idx = None; # Index of the entry in the trimmed list of the `lea REG, [RIP - 0xXYZZY]`
    self.recoverable_next_rip = None;
    self.recoverable_jmp_reg = None;
  #end-def
  ...
  def recover(self):
    if self.recoverable:
      self.rip = self.recoverable_next_rip;
      self.chunk_idx += 1;

      self.trimmed[self.recover_trimmed_lea_idx] = ('lea %s, [shellcode_chunk_%d]' % (self.engine.reg_name(~self.recoverable_jmp_reg), self.chunk_idx));
      self.trimmed.append('shellcode_chunk_%d:' % self.chunk_idx); # Label for NASM

      # Clear the recoverable status
      self.recoverable = False;
      self.recover_trimmed_lea_idx = None;
      self.recoverable_next_rip = None;
      self.recoverable_jmp_reg = None;

      # Also trim the cached instructions or the memory allocation will blow up.
      del self.cached_instructions[0 : -0x20];
      for i in range(len(self.trimmed)):
        item = self.trimmed[i];
        if type(item) != str:
          self.trimmed[i] = ('%s %s' % (item.mnemonic, item.op_str));
        #end-if
      #end-for
    ...
    match opc:
      case 'hlt':
        ...
        match unwind_scheme:
          ...
          case 0xF30:
            ...
            print('[UNWIND-LIFTED] %s' % s);
          case _:
            raise Exception('Unexpected unwind operation sequence/scheme %X at offset %08X! Abort!' % (unwind_scheme, unwind_info_offset));
          #end-cases
        #end-match
      ...
      case 'jmp':
        ...
        if ins.bytes[0] == 0xE9: # Direct relative call
          jmp_dst = op_dst.value.imm;
          self.rip = jmp_dst - self.base;

          del self.trimmed[-1:];

          print('Updated RIP to the JMP to 0x%08x' % jmp_dst);
        elif (self.recoverable_jmp_reg < 0) and (op_dst.type == X86_OP_REG) and (op_dst.reg == ~self.recoverable_jmp_reg):
          # Recoverable
          self.recoverable = True;
          raise Exception('Found a JMP to potentially recoverable address! Abort to dump the cached buffer!');
        else:
      ...
      case 'lea':
        lhs = ins.operands[0];
        rhs = ins.operands[1];
        # I use negative ID value to set the "locked" status from `cmovne`
        if (rhs.type == X86_OP_MEM) and (rhs.mem.base == X86_REG_RIP) and (rhs.mem.scale == 1) and ((not self.recoverable_jmp_reg) or (self.recoverable_jmp_reg >= 0)):
          self.recoverable_next_rip = self.rip + rhs.mem.disp;
          self.recoverable_jmp_reg = lhs.reg;

          self.recover_trimmed_lea_idx = len(self.trimmed) - 1;

          print('[DEOBF RECOVERING DETECTION] detected potential recoverable address: 0x%08X' % (self.recoverable_next_rip + self.base));
        #end-def
      case 'ldmxcsr':
      ...
      case 'cmovne':
        lhs = ins.operands[0];
        rhs = ins.operands[1];
        if (lhs.type == X86_OP_REG) and (rhs.type == X86_OP_REG) and (lhs.reg == self.recoverable_jmp_reg):
          self.recoverable_jmp_reg = ~self.recoverable_jmp_reg;
        #end-if
      case _:
        pass;
      #end-cases
...
def __main__():
  ...
  while True:
    ...
    fp = open('dumps/shellcode-deobf03-%02d.bin' % parser.chunk_idx, 'wb');
    fp.write(parser.shellcode);
    fp.close();

    if not parser.recover():
      print('Deobfuscator is now in a unrecoverable state. Halt!');
      break;
    #end-if
  #end-while

  fp = open('deobf03.txt', 'w');
